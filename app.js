...
function activeViewName() {
  ...
  return active?.id?.replace(/^view-/, "") || "projections";
}

function projectionFilterState() {
  return window.hammerProjectionFilters?.getState?.() ?? {};
}

function captureNavigationState(viewName = activeViewName()) {
  return {
    hammerNavigation: true,
    view: viewName,
    ...
    projectionFilters: projectionFilterState()
  };
}
...
function restoreNavigationState(state) {
  ...
  if (state.view === "projections") {
    window.hammerProjectionFilters?.setState?.(
      state.projectionFilters || {},
      { render: false }
    );
  ...
  } else if (state.view === "ratings") {
    renderRatings();
  }

  switchView(state.view || "projections", {
    scrollY: Number(state.scrollY || 0),
    updateHistory: false
  });
}

function detailHash(viewName, value) {
  return `#${viewName}=${encodeURIComponent(String(value || ""))}`;
}

function updateDetailBackButton(viewName, returnView) {
  const view = document.getElementById(`view-${viewName}`);
  const button = view?.querySelector(".back-button");
  if (!button) return;
  ...
  const destination = destinations[returnView] ? returnView : "projections";
  button.textContent = `← Back to ${destinations[destination]}`;
  button.setAttribute("onclick", `returnFromDetail('${destination}')`);
}

function enterDetailView(viewName, detail = {}) {
  const returnState = captureNavigationState();
  ...
  const key = detail.gameId || detail.teamName || "detail";
  history.pushState(detailState, "", detailHash(viewName, key));
  updateDetailBackButton(viewName, returnState.view);
  switchView(viewName, { updateHistory: false });
}

function returnFromDetail(fallbackView = "projections") {
  if (history.state?.returnState) {
    history.back();
    return;
  }
  ...
  restoreNavigationState(
    detailReturnState || {
      ...captureNavigationState(fallbackView),
      view: fallbackView,
      scrollY: 0
    }
  );
...
window.addEventListener("popstate", event => {
  ...
  if (!state?.hammerNavigation) {
    restoreNavigationState(detailReturnState || {
      ...captureNavigationState("projections"),
      view: "projections",
      scrollY: 0
    });
    return;
  ...
  if (state.view === "matchup" && state.detail?.gameId) {
    const game = findGame(state.detail.gameId);
    if (game) renderMatchup(game);
    detailReturnState = state.returnState || detailReturnState;
    updateDetailBackButton("matchup", detailReturnState?.view);
    switchView("matchup", { updateHistory: false, scrollY: state.scrollY || 0 });
    return;
  }

  if (state.view === "dossier" && state.detail?.teamName) {
    const team = getTeam(state.detail.teamName);
    if (team) {
    ...
    switchView("dossier", { updateHistory: false, scrollY: state.scrollY || 0 });
    return;
  }
...
function switchView(viewName, options = {}) {
  ...
  const targetScroll = Number.isFinite(Number(options.scrollY))
...
function ensureMatchupView() {
  if (!document.getElementById("view-matchup")) {
    ...
    if (main) {
      ...
      section.innerHTML = `
        <button class="back-button" onclick="returnFromDetail('projections')">
          ← Back to projections
        </button>
  ...
  style.textContent = `
    ...
    .thi-matchup-section-title {
      font-size:15px;
      font-weight:800;
      letter-spacing:-.25px;
    }

    .thi-matchup-section-note {
      color:var(--muted);
      font-family:var(--mono);
      font-size:8px;
      line-height:1.5;
    ...
    .thi-tale-grid {
      display:grid;
      grid-template-columns:minmax(180px,.72fr) minmax(320px,1.4fr) minmax(320px,1.4fr) minmax(180px,.72fr);
      gap:10px;
      padding:10px;
    }
    ...
    .thi-profile-stat:last-child { border-bottom:none; }
    .thi-profile-label { color:var(--muted); font-size:10px; }
    .thi-profile-value { font-family:var(--mono); font-size:10px; font-weight:800; }
    ...
    .thi-matchup-metric-row {
      display:grid;
      grid-template-columns:minmax(72px,.82fr) minmax(105px,1.1fr) minmax(72px,.82fr);
      align-items:stretch;
      min-height:38px;
      border-bottom:1px solid #eeeeeb;
    }

    .thi-matchup-metric-row:last-child { border-bottom:none; }

    .thi-matchup-metric-label {
      display:flex;
      align-items:center;
      justify-content:center;
      padding:6px 5px;
    ...
    .thi-matchup-metric-value:first-child { flex-direction:row-reverse; }

    .thi-band-elite { background:#087f5b; color:#fff; }
    .thi-band-strong { background:#49b783; color:#092d22; }
    .thi-band-above { background:#c9f2dd; color:#174536; }
    .thi-band-average { background:#f1f2ee; color:#4c5660; }
    .thi-band-below { background:#fff0bd; color:#74520a; }
    ...
    .thi-context-fact:last-child { border-right:none; }
    .thi-context-label {
      color:var(--muted);
      font-family:var(--mono);
      font-size:7px;
    ...
    @media (max-width:900px) {
      ...
      .thi-unit-matchup:nth-child(3) { order:4; }
    ...
    @media (max-width:520px) {
      ...
      .thi-context-fact:last-child { border-bottom:none; }
      .thi-team-summary:first-child,
...
function profileMetric(teamName, side, field) {
  return teamMetricProfile(teamName)?.[side]?.[field] ?? null;
}

function profileMetricDefinition(field) {
  return teamMetricProfilesData?.meta?.metrics?.[field] ?? {};
}

function profileMetricText(metric, field) {
  const value = metric?.value;
  if (!hasValue(value)) return "—";
  const format = profileMetricDefinition(field)?.format;
  if (format === "percent") return formatPercent(value, 1);
  if (format === "epa") return formatSigned(value, 3);
  return formatNumber(value, 2);
}

function profileRankBadge(metric) {
  const rank = hasValue(metric?.rank) ? `#${Number(metric.rank)}` : "—";
  const band = metric?.band || "missing";
  const percentile = hasValue(metric?.percentile)
    ? `${formatNumber(metric.percentile, 0)}th percentile`
    : "Sample developing";
  return `
    <span
      class="thi-metric-rank thi-band-${escapeHtml(band)}"
      title="${escapeHtml(percentile)}"
    >${escapeHtml(rank)}</span>
  `;
}
...
function teamSummaryMetric(label, metric, field) {
  return `
    <div class="thi-profile-stat">
      <span class="thi-profile-label">${escapeHtml(label)}</span>
      <span class="thi-profile-value">${escapeHtml(profileMetricText(metric, field))}</span>
      ${profileRankBadge(metric)}
    </div>
  `;
}

function teamSummaryCard(teamName) {
  const profile = teamMetricProfile(teamName);
  const net = profile?.net_epa ?? null;
  const offenseEpa = profileMetric(teamName, "offense", "epa_play");
  const defenseEpa = profileMetric(teamName, "defense", "epa_play");
  const offenseSuccess = profileMetric(teamName, "offense", "success_rate");
  const defenseSuccess = profileMetric(teamName, "defense", "success_rate");
  const sample = profile?.sample ?? {};
  const sampleNote = `${Number(sample.offense_plays || 0)} OFF · ${Number(sample.defense_plays || 0)} DEF plays`;

  return `
    <div class="thi-team-summary">
      <div class="thi-profile-heading">
        ${teamLogoMarkup(teamName, "projection")}
        <span>${escapeHtml(teamName)}</span>
      </div>
      <div class="thi-profile-sample">${escapeHtml(sampleNote)}</div>
      ${teamSummaryMetric("Net EPA / Play", net, "epa_play")}
      ${teamSummaryMetric("Offensive EPA", offenseEpa, "epa_play")}
      ${teamSummaryMetric("Defensive EPA", defenseEpa, "epa_play")}
      ${teamSummaryMetric("Offensive Success", offenseSuccess, "success_rate")}
      ${teamSummaryMetric("Defensive Success", defenseSuccess, "success_rate")}
      <div class="thi-profile-stat">
        <span class="thi-profile-label">Power Rank</span>
        <span class="thi-profile-value">${escapeHtml(profile?.power_rank ? `#${profile.power_rank}` : "—")}</span>
        <span class="thi-metric-rank thi-band-average" title="Frozen Model A power rank">MODEL</span>
      </div>
    </div>
  `;
}
...
function unitMetricCell(teamName, side, field) {
  ...
  const band = metric?.band || "missing";
  return `
    <div class="thi-matchup-metric-value thi-band-${escapeHtml(band)}">
      <span>${escapeHtml(profileMetricText(metric, field))}</span>
      <span>${hasValue(metric?.rank) ? `#${Number(metric.rank)}` : "—"}</span>
    </div>
  `;
}
...
function unitMatchupCard(offenseTeam, defenseTeam) {
  ...
  return `
    <div class="thi-unit-matchup">
      <div class="thi-unit-heading">
        <span>${escapeHtml(offenseTeam)} Offense</span>
        <span class="thi-unit-versus">vs</span>
        <span>${escapeHtml(defenseTeam)} Defense</span>
      </div>
      ${fields.map(field => `
        <div class="thi-matchup-metric-row">
          ${unitMetricCell(offenseTeam, "offense", field)}
          <div class="thi-matchup-metric-label">
            ${escapeHtml(profileMetricDefinition(field)?.label || field)}
          </div>
          ${unitMetricCell(defenseTeam, "defense", field)}
        </div>
      `).join("")}
    </div>
  `;
...
function taleOfTapeMarkup(awayName, homeName) {
  ...
  const sample = teamMetricProfilesData?.meta?.sample === "non_garbage"
    ...
    : "all plays";
  return `
    <section class="thi-matchup-section" aria-labelledby="thi-tale-title">
      <div class="thi-matchup-section-header">
        <div>
          <div class="eyebrow">Matchup profile</div>
          <div class="thi-matchup-section-title" id="thi-tale-title">Tale of the Tape</div>
        </div>
        <div>
          <div class="thi-matchup-section-note">
            2026 season to date · through Week ${escapeHtml(throughWeek ?? "—")} · ${escapeHtml(sample)}
          </div>
          <div class="thi-color-legend" aria-label="Metric percentile color scale">
            <span>Weak</span><span class="thi-color-legend-bar"></span><span>Strong</span>
          </div>
        </div>
      </div>
      <div class="thi-tale-grid">
        ${teamSummaryCard(awayName)}
        ${unitMatchupCard(awayName, homeName)}
...
function scheduleContextForGame(game) {
  const gameId = String(game?.game_id ?? game?.id ?? "");
  if (gameId && scheduleContextData?.games?.[gameId]) {
    return scheduleContextData.games[gameId];
  }

  const awayName = game?.away?.team;
  const homeName = game?.home?.team;
  const week = Number(game?.week);
  return Object.values(scheduleContextData?.games ?? {}).find(item =>
    Number(item?.week) === week &&
    item?.away_team === awayName &&
    item?.home_team === homeName
  ) ?? null;
}
...
function scheduleOpponentText(item, kind) {
  ...
  if (kind === "previous_game" && game?.result) {
    return `${game.result} ${game.score ?? ""} vs ${game.opponent}`.trim();
  }
  const rank = Number(game?.opponent_power_rank);
...
function scheduleContextCard(item) {
  ...
  return `
    <div class="thi-context-card">
      <div class="thi-context-team">${escapeHtml(item?.team ?? "Team")}</div>
      <div class="thi-context-facts">
        ...
        <div class="thi-context-fact">
          ...
          <div class="thi-context-value">${escapeHtml(scheduleOpponentText(item, "previous_game"))}</div>
        </div>
        <div class="thi-context-fact">
          <div class="thi-context-label">Next</div>
          <div class="thi-context-value">${escapeHtml(scheduleOpponentText(item, "next_game"))}</div>
        </div>
      </div>
      <div class="thi-context-flags">
        ${flags.length
          ? flags.map(flag => `<span class="thi-context-flag">${escapeHtml(flag?.label ?? "Context flag")}</span>`).join("")
          : `<span class="thi-context-clear">No elevated schedule-spot flag identified.</span>`}
      </div>
    </div>
  `;
}

function scheduleContextMarkup(game) {
  ...
  container.innerHTML = `
    ...
    <div class="analysis-grid">
      <div class="analysis-card">
        <div class="analysis-label">THI Fair Line</div>
      ...
      <div class="analysis-card">
        <div class="analysis-label">THI Total</div>
...
function renderRatings() {
  ...
  const marketRows = data
    .map(team => ({
      team,
      market: teamMarketPerformanceData?.teams?.[team.team] ?? null,
    }))
    .filter(row => row.market?.overall?.games > 0)
    .sort((a, b) =>
      Number(b.market?.overall?.average_cover_margin ?? -999) -
      Number(a.market?.overall?.average_cover_margin ?? -999)
    );
  ...
  const marketTable = `
    <div class="ratings-note">
      ...
      Average Cover Margin compares each team's actual scoring margin with the
      near-kickoff market expectation. Positive is market outperformance;
      negative is market underperformance. This is descriptive tracking, not a
      betting recommendation. Teams need at least two tracked games for an
      ...
      <table class="projection-table">
        <thead>
          <tr>
            ...
            <th>ATS Record</th>
            <th>ATS Win %</th>
            <th>Avg Cover Margin</th>
            <th>Median Cover Margin</th>
            <th>Home</th>
            <th>Away</th>
            <th>Favorite</th>
            <th>Underdog</th>
        ...
        <tbody>
          ${marketRows.map(row => {
            const overall = row.market.overall || {};
            const margin = Number(overall.average_cover_margin);
            const band = !Number.isFinite(margin)
              ? "missing"
              : margin >= 7
                ? "elite"
                : margin >= 3
            ...
            return `
              <tr style="cursor:pointer" onclick="openDossier('${escapeJsString(row.team.team)}')">
                <td class="team-meta">${row.market.market_rank ? `#${row.market.market_rank}` : "—"}</td>
                <td>
                  <span class="team-with-logo">
                    ${teamLogoMarkup(row.team.team)}
                    <strong>${escapeHtml(row.team.team)}</strong>
                ...
                <td class="team-meta">${escapeHtml(row.team.conference ?? "—")}</td>
                <td class="line-primary">${escapeHtml(overall.record ?? "—")}</td>
                <td class="team-meta">${formatPercent(overall.win_pct_ex_pushes)}</td>
                <td><span class="thi-metric-rank thi-band-${band}">${formatSigned(margin, 2)}</span></td>
                <td class="team-meta">${formatSigned(overall.median_cover_margin, 2)}</td>
                <td class="team-meta">${escapeHtml(row.market.home?.record ?? "—")}</td>
                <td class="team-meta">${escapeHtml(row.market.away?.record ?? "—")}</td>
                <td class="team-meta">${escapeHtml(row.market.favorite?.record ?? "—")}</td>
                <td class="team-meta">${escapeHtml(row.market.underdog?.record ?? "—")}</td>
                <td class="team-meta">${escapeHtml(row.market.market_label ?? "SAMPLE DEVELOPING")}</td>
              </tr>
            `;
          }).join("")}
        </tbody>
