(() => {
  ...
  function ensureFilterUI() {
    ...
    wrapper.addEventListener("click", event => {
      ...
      activeStatus = normalizedStatus(button.dataset.status);
      applyFilterAndUI();
  ...
  // Public, presentation-only state hook used by the mobile card adapter and
  // SPA history restoration. This never changes projection or grading data.
  window.hammerStatusControls = {
    getActiveStatus() {
      return activeStatus;
    },
    setActiveStatus(status, options = {}) {
      activeStatus = normalizedStatus(status);
      applyFilterAndUI();
      if (options.emit !== false) announceStatusChange();
    }
  };
