...
Production HFA version:
- Strict FBS-vs-FBS projections
- Full team schedules including FCS opponents
- Per-game win probabilities
- Completed results locked as actual wins/losses
- Exact season win distributions and alternate win totals
- Team-specific 2026 home-field advantage from data/hfa_2026.json
- Neutral sites always receive 0.0 HFA
- HFA coverage is validated before projections are built

IMPORTANT:
FCS opponents are NEVER mapped to similarly named FBS teams.
"""
...
def resolve_cfbd_team(provider_name, valid_teams, lookup):
    """Strict school-name matching. No prefix matching."""
...
def cfbd_get(endpoint, params=None, required=True):
    ...
    try:
        ...
        print(f"❌ CFBD request failed: {error}")
        if required:
            sys.exit(1)
        return []
    ...
    try:
        ...
        print("❌ CFBD returned invalid JSON.")
        if required:
            sys.exit(1)
        return []
...
def load_hfa(valid_teams):
    ...
    if extra:
        print(f"⚠ HFA file contains {len(extra)} extra team(s) not in the current model")
...
def calculate_rating_scale(teams):
    ...
    if len(x_values) < 10:
        return {"slope": 7.5, "intercept": 0.0, "method": "fallback"}
    ...
    slope = numerator / denominator if denominator else 7.5
    intercept = y_mean - slope * x_mean

    print("")
    print("📐 Power rating calibration")
    print(f"   1 rating unit = {slope:.3f} points")
...
def extract_market(raw_game):
    ...
    if selected is None:
        return {"spread": None, "total": None, "bookmaker": None}
...
def fetch_odds(valid_teams, lookup):
    ...
    for raw in raw_games:
        ...
        home = resolve_odds_team(raw_home, valid_teams, lookup)
        away = resolve_odds_team(raw_away, valid_teams, lookup)
...
def fetch_schedule(valid_teams, lookup):
    ...
    for raw in raw_games:
        ...
        neutral_site = bool(
            first_value(raw, "neutralSite", "neutral_site", default=False)
        )
        ...
        unique_key = (
            ...
            else (raw_home, raw_away, start_date)
        )
        ...
        if home_fbs and away_fbs:
            game = {
                ...
                "home_conference": first_value(
                    raw, "homeConference", "home_conference"
                ),
                "away_conference": first_value(
                    raw, "awayConference", "away_conference"
                ),
                "venue": venue,
    ...
    full_entries = sum(len(games) for games in full_team_schedule.values())
    fcs_entries = sum(
        1
...
def live_section(team, side):
    return ((team.get(side, {}) or {}).get("live_2026", {}) or {})
...
def both_have_general_sample(home, away):
    return live_has_general_sample(home) and live_has_general_sample(away)
...
def calculate_matchup_adjustment(home, away):
    ...
    availability = {key: False for key in components}
    ...
    if both_have_pass_sample(home, away):
        ...
        if all(value is not None for value in values):
            ...
            components["passing"] = bounded((home_edge - away_edge) * 1.25, 1.0)
    ...
    if both_have_rush_sample(home, away):
        ...
        if all(value is not None for value in values):
            ...
            components["rushing"] = bounded(
                (home_rush - away_rush_def) - (away_rush - home_rush_def),
                0.75,
            )
    ...
    if both_have_general_sample(home, away):
        ...
        if all(value is not None for value in success_values):
            ...
            components["success_rate"] = bounded(
                ((home_sr - away_def_sr) - (away_sr - home_def_sr)) * 0.035,
                0.75,
            )
        ...
        if all(value is not None for value in explosive_values):
            home_explosive, away_def_explosive, away_explosive, home_def_explosive = explosive_values
        ...
        if all(value is not None for value in havoc_values):
            home_allowed, home_created, away_allowed, away_created = havoc_values
            components["havoc"] = bounded(
                ((home_created - away_allowed) - (away_created - home_allowed)) * 0.025,
                0.50,
            )
            availability["havoc"] = True

    total = bounded(sum(components.values()), MAX_MATCHUP_ADJUSTMENT)
    ...
    return {
        ...
        "note": (
            ...
            if comparable
            else "No comparable 2026 live sample; matchup adjustment held at zero."
        ),
    }
...
def calculate_total(home, away):
    ...
    total = BASE_TOTAL + efficiency * 5.0 + success * 0.15
...
def calculate_win_probability(home_spread):
    ...
    probability = normal_cdf(home_margin / WIN_PROB_STD_DEV)
    probability = max(0.01, min(0.99, probability))

    return {
        "home": round(probability * 100, 1),
        "away": round((1.0 - probability) * 100, 1),
        "method": f"Normal margin distribution, sigma={WIN_PROB_STD_DEV:.1f}",
    }


def fcs_win_probability(team_power_rating, rating_slope, hfa_points=0.0):
    """
    ...
    its production HFA is then added. Neutral/away FBS games receive 0.0.
    """
    ...
    expected_margin = FCS_BASE_MARGIN + power_points + safe_number(hfa_points)

    probability = normal_cdf(expected_margin / WIN_PROB_STD_DEV)
    probability = max(FCS_MIN_WIN_PROB, min(FCS_MAX_WIN_PROB, probability))

    return {
        "probability": round(probability * 100, 1),
        "expected_margin": round(expected_margin, 1),
        "home_field_advantage": round(safe_number(hfa_points), 1),
...
def adjusted_status_threshold(market_spread):
    if market_spread is None:
        return {"play": None, "watch": None}

    magnitude = abs(market_spread)

    if magnitude >= 35:
        return {"play": 10.0, "watch": 6.0}

    if magnitude >= 21:
        return {"play": 8.5, "watch": 5.0}

    return {"play": 7.0, "watch": 4.0}


def compare_to_market(model_spread, market_spread, home_name, away_name):
    if market_spread is None:
        return {
            "disagreement": None,
            "preferred_side": None,
    ...
    thresholds = adjusted_status_threshold(market_spread)
...
def generate_insights(
    ...
    if home_rank and away_rank and home_rank != away_rank:
        ...
        insights.append({
            ...
            "text": f"{stronger} owns the stronger overall blended power rating.",
            "source": "power_rating",
        })
    ...
    if not matchup["comparable_live_sample"]:
        insights.append({
            ...
            "text": (
                "No live matchup adjustment is being applied because both teams "
                "do not yet have comparable 2026 samples."
            ),
            "source": "sample_guard",
        })
    else:
        usable = [
            ...
            if matchup["available"].get(key) and abs(value) >= 0.15
        ]
        usable.sort(key=lambda item: abs(item[1]), reverse=True)
        ...
        for key, value in usable[:2]:
            ...
            insights.append({
                ...
                "text": f"The comparable 2026 sample leans toward {favored}.",
                "source": key,
            })
    ...
    if market_spread is not None:
        ...
        if abs(difference) >= 1.5:
            ...
            insights.append({
                ...
                "text": (
                    f"The model is {abs(difference):.1f} points more favorable "
                    f"to {favored} than the current market."
                ),
                "source": "market",
            })
...
def build_projections(teams, schedule, odds, calibration, hfa_data):
    print("")
    ...
    for game in schedule["games"]:
        ...
        hfa = get_team_hfa(home_name, hfa_data, neutral=neutral)
        ...
        model_spread = round_half(spread_after_hfa - matchup["total"])
        ...
        projections.append({
            ...
            "projection": {
                ...
                "components": {
                    ...
                    "rating_only_home_spread": round_half(rating_home_spread),
                    ...
                    "spread_after_home_field": round_half(spread_after_hfa),
    ...
    print(f"✅ Projections with market match: {market_matches}")
    print(
...
def probability_distribution(probabilities):
    ...
    for probability in probabilities:
        ...
        next_distribution = [0.0 for _ in range(len(distribution) + 1)]

        for wins, current_probability in enumerate(distribution):
            next_distribution[wins] += current_probability * (1.0 - p)
            next_distribution[wins + 1] += current_probability * p
...
def alt_total_probabilities(distribution):
    ...
    for wins in range(4, max_wins + 1):
        line = wins + 0.5
        over_probability = sum(distribution[wins + 1:])
...
def build_projection_lookup(projections):
    return {str(game.get("game_id")): game for game in projections}
...
def build_season_projections(
    ...
    for team_name, team in teams.items():
        ...
        for game in games:
            completed_result = completed_team_result(team_name, game)
            hfa_used = None
            ...
            if completed_result is not None:
                ...
                projection = game_lookup.get(str(game.get("id")))
                ...
                if projection["home"]["team"] == team_name:
                    probability = (
                        projection["projection"]["win_probability"]["home"] / 100.0
                    )
                    model_line = projection["projection"]["home_spread"]
                else:
                    probability = (
                        projection["projection"]["win_probability"]["away"] / 100.0
                    )
                    model_line = -projection["projection"]["home_spread"]

                hfa_used = projection["projection"].get("home_field_advantage")
                source = "fbs_model"

            else:
                neutral = bool(game.get("neutral_site", False))
                is_true_home = game.get("location") == "home" and not neutral

                hfa_used = (
                    get_team_hfa(team_name, hfa_data, neutral=False)
                    if is_true_home
                    else 0.0
                )
                ...
                probability = fallback["probability"] / 100.0
                source = "fcs_fallback"
                model_line = -fallback["expected_margin"]
        ...
        distribution = probability_distribution(game_probabilities)
        ...
        most_likely_losses = games_count - most_likely_wins

        at_least = {}
        for target in (6, 8, 9, 10, 11, 12):
            at_least[str(target)] = round(
                probability_at_least(distribution, target) * 100,
                1,
            )
        ...
        season_output[team_name] = {
            ...
            "alt_win_totals": alt_total_probabilities(distribution),
            "schedule": schedule_projection,
        }

    print(f"✅ Season projections built: {len(season_output)} teams")
...
def validate_projection_output(projections, season_projections):
    ...
    for team, data in season_projections.items():
        distribution = data.get("exact_win_distribution", {})
        total = sum(safe_number(value) for value in distribution.values())

        if abs(total - 100) > 0.5:
            bad_distributions.append((team, total))

    if bad_distributions:
        print("❌ Win distribution sanity check failed.")
...
def main():
    ...
    lookup = build_team_lookup(valid_teams)

    # Production HFA is validated before any projections are built.
    hfa_data = load_hfa(valid_teams)

    calibration = calculate_rating_scale(teams)

    odds = fetch_odds(valid_teams, lookup)
    save_json(ODDS_PATH, odds)

    schedule = fetch_schedule(valid_teams, lookup)
    ...
    validate_projection_output(projections, season_projections)

    hfa_meta = hfa_data.get("meta", {}) or {}
    ...
    output = {
        "meta": {
            ...
            "teams_with_season_projection": len(season_projections),
            "version": "3.1-team-specific-hfa",
            "calibration": calibration,
            "home_field_advantage": {
                "type": "team_specific",
                "source": HFA_PATH,
                "version": hfa_meta.get("version"),
                "default": safe_number(hfa_meta.get("default_hfa"), 2.0),
                "minimum": safe_number(hfa_meta.get("minimum_hfa"), 1.0),
                "maximum": safe_number(hfa_meta.get("maximum_hfa"), 3.5),
            ...
            "max_matchup_adjustment": MAX_MATCHUP_ADJUSTMENT,
            "win_probability_std_dev": WIN_PROB_STD_DEV,
            "fcs_model": {
                "base_margin": FCS_BASE_MARGIN,
                "power_multiplier": FCS_POWER_MULTIPLIER,
                "minimum_win_probability": FCS_MIN_WIN_PROB * 100,
                "maximum_win_probability": FCS_MAX_WIN_PROB * 100,
                "home_hfa_rule": (
                    "FBS team-specific HFA added only when the FBS team is "
                    "the true home team; neutral/away receives 0.0."
                ),
                "note": "Generic fallback used only for season-win projections.",
            },
            "live_matchup_rules": {
                "minimum_plays": MIN_LIVE_PLAYS,
                "minimum_pass_plays": MIN_LIVE_PASS_PLAYS,
                "minimum_rush_plays": MIN_LIVE_RUSH_PLAYS,
                "requires_both_teams": True,
            },
            "metrics_through_week": (
                metrics_data.get("meta", {}).get("through_week")
            ),
        },
        "games": projections,
    ...
    save_json(PROJECTIONS_PATH, output)

    print("")
    print("=" * 70)
    print("🎉 PROJECTION BUILD COMPLETE")
    print("=" * 70)
    print(f"FBS-vs-FBS games: {len(projections)}")
    print(f"Season projections: {len(season_projections)} teams")
