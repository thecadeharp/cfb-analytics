...
def resolve_cfbd_team(provider_name, valid_teams, lookup):
    """Strict FBS school matching for CFBD. No prefix matching."""
...
def collect_fcs_games(schedule):
    """
    FBS-v-FCS games live only in the FBS team_schedules branch.
    Return one unique game per schedule id/matchup.
    """
    output = []
    seen = set()

    for games in (schedule.get("team_schedules", {}) or {}).values():
        for game in games:
            ...
            key = (
                ...
                if game_id is not None
                    ...
                    canonical_name(game.get("away_team")),
                    canonical_name(game.get("home_team")),
                    str(game.get("start_date") or ""),
                )
            )
    ...
    output.sort(
        key=lambda game: (
            game.get("week") if game.get("week") is not None else 99,
...
def build_fcs_lookup(schedule):
    """
    Build a lookup from the actual FCS opponents on the FBS schedule.

    This is what prevents sportsbook labels such as:
      Arkansas Pine Bluff Golden Lions
    from being incorrectly resolved as:
      Arkansas
    """
    lookup = {}

    for game in collect_fcs_games(schedule):
        for name in (
            game.get("opponent"),
            game.get("home_team"),
            game.get("away_team"),
            ...
            # Do not classify the FBS side as FCS.
...
def resolve_odds_participant(
    ...
    """
    Resolve sportsbook labels against BOTH known FBS schools and the scheduled
    FCS opponent universe, then choose the longest matching school name.

    Longest-match behavior fixes collisions such as:
      Arkansas Pine Bluff -> Arkansas
      North Carolina A&T -> North Carolina
    while still allowing mascot suffixes such as:
    ...
    # Exact FBS first.
    if canon in fbs_lookup:
        return {
            "name": fbs_lookup[canon],
            "level": "FBS",
        }
    ...
    # Exact scheduled FCS.
    if canon in fcs_lookup:
        return {
            "name": fcs_lookup[canon],
            "level": "FCS",
        }

    candidates = []

    for school_canon, school_name in fbs_lookup.items():
        if canon.startswith(school_canon + " "):
            candidates.append(
                (len(school_canon), school_name, "FBS")
            )

    for school_canon, school_name in fcs_lookup.items():
        if canon.startswith(school_canon + " "):
            candidates.append(
                (len(school_canon), school_name, "FCS")
            )

    # Aliases can also be the start of sportsbook labels.
    for alias_canon, model_name in ALIASES.items():
        if model_name not in valid_teams:
            continue
        if canon.startswith(alias_canon + " "):
            candidates.append(
                (len(alias_canon), model_name, "FBS")
            )
...
def load_cached_schedule():
    ...
    print(f"✅ Cached FBS-vs-FCS games: {len(collect_fcs_games(schedule))}")
    return schedule
...
def fetch_schedule(valid_teams, lookup):
    ...
    fbs_fbs_games = []
    full_team_schedule = {team: [] for team in valid_teams}

    rejected_non_fbs = 0
    seen = set()

    for raw in raw_games:
        raw_home = first_value(raw, "homeTeam", "home_team")
        raw_away = first_value(raw, "awayTeam", "away_team")

        home_fbs = resolve_cfbd_team(raw_home, valid_teams, lookup)
        away_fbs = resolve_cfbd_team(raw_away, valid_teams, lookup)

        game_id = first_value(raw, "id")
        start_date = first_value(raw, "startDate", "start_date")
        ...
        home_points = first_value(raw, "homePoints", "home_points")
        away_points = first_value(raw, "awayPoints", "away_points")
        completed = first_value(raw, "completed", default=False)
    ...
    print(f"✅ TRUE FBS-vs-FBS games: {len(fbs_fbs_games)}")
    print(f"✅ Team schedule entries: {full_entries}")
    print(f"✅ FBS-vs-FCS team entries: {fcs_entries}")
...
def market_consensus(quotes, outlier_distance):
    if not quotes:
        return {
            "value": None,
            "books": 0,
    ...
    if len(quotes) >= MARKET_MIN_BOOKS_FOR_FILTERING:
        filtered = [
            quote
            for quote in quotes
            if abs(quote["value"] - initial_median) <= outlier_distance
        ]
    ...
    final_value = statistics.median(
        [quote["value"] for quote in filtered]
    )

    return {
        "value": round_half(final_value),
        "books": len(filtered),
        "raw_books": len(quotes),
        "excluded": excluded,
        "quotes": filtered,
    }
...
def extract_market(raw_game):
    ...
    for book in bookmakers:
        ...
        book_title = book.get("title") or book_key or "Unknown"
    ...
    total_consensus = market_consensus(
        total_quotes,
        MARKET_TOTAL_OUTLIER_DISTANCE,
    )

    if spread_consensus["excluded"]:
        print(
            "⚠ Spread outlier(s) removed: "
            + ", ".join(
                f"{quote['bookmaker']} {quote['value']:+.1f}"
                for quote in spread_consensus["excluded"]
            )
        )

    if total_consensus["excluded"]:
        print(
            "⚠ Total outlier(s) removed: "
            + ", ".join(
                f"{quote['bookmaker']} {quote['value']:.1f}"
    ...
    spread_books = spread_consensus["books"]
    ...
    return {
        ...
        "bookmaker": (
            f"Consensus ({market_books} books)"
            if market_books > 0
            else None
        ),
        "spread_books": spread_books,
        "total_books": total_books,
        "spread_raw_books": spread_consensus["raw_books"],
        "total_raw_books": total_consensus["raw_books"],
...
def fetch_odds(
    ...
    for raw in raw_games:
        ...
        home = resolve_odds_participant(
            raw_home,
            valid_teams,
            fbs_lookup,
            fcs_lookup,
        )
        away = resolve_odds_participant(
            raw_away,
            valid_teams,
            fbs_lookup,
            fcs_lookup,
        ...
        home_level = home["level"] if home else None
        away_level = away["level"] if away else None

        if home and away:
            levels = {home_level, away_level}
            if levels == {"FBS"}:
                fbs_fbs_matched += 1
            elif levels == {"FBS", "FCS"}:
                fbs_fcs_matched += 1
        ...
        games.append({
            ...
            "spread_books": market["spread_books"],
            "total_books": market["total_books"],
            "spread_raw_books": market["spread_raw_books"],
    ...
    print(f"   FBS-vs-FBS market matches: {fbs_fbs_matched}")
    print(f"   FBS-vs-FCS market matches: {fbs_fcs_matched}")
    ...
    return {
        "meta": {
            ...
            "fbs_fbs_matched_games": fbs_fbs_matched,
            "fbs_fcs_matched_games": fbs_fcs_matched,
            "source": "The Odds API",
            "participant_matching": "longest_known_school_match_fbs_plus_scheduled_fcs",
            "market_method": "cross_book_median_consensus",
            "spread_outlier_distance": MARKET_SPREAD_OUTLIER_DISTANCE,
            "total_outlier_distance": MARKET_TOTAL_OUTLIER_DISTANCE,
            "minimum_books_for_filtering": MARKET_MIN_BOOKS_FOR_FILTERING,
        },
        "games": games,
    }
...
def live_has_general_sample(team):
    ...
    return (
        safe_number(offense.get("n_plays")) >= MIN_LIVE_PLAYS
        and safe_number(defense.get("n_plays")) >= MIN_LIVE_PLAYS
    )
...
def both_have_pass_sample(home, away):
    for team in (home, away):
        ...
        if safe_number(offense.get("pass_plays")) < MIN_LIVE_PASS_PLAYS:
            return False

        if safe_number(defense.get("pass_plays")) < MIN_LIVE_PASS_PLAYS:
            return False
...
def both_have_rush_sample(home, away):
    for team in (home, away):
        ...
        if safe_number(offense.get("rush_plays")) < MIN_LIVE_RUSH_PLAYS:
            return False

        if safe_number(defense.get("rush_plays")) < MIN_LIVE_RUSH_PLAYS:
            return False
...
def empty_matchup_adjustment(note):
    return {
        "total": 0.0,
        "components": {
            "passing": 0.0,
...
def calculate_matchup_adjustment(home, away):
    ...
    if both_have_pass_sample(home, away):
        ...
        if all(value is not None for value in values):
            home_pass, away_pass_def, away_pass, home_pass_def = values
    ...
    if both_have_rush_sample(home, away):
        ...
        if all(value is not None for value in values):
            home_rush, away_rush_def, away_rush, home_rush_def = values
    ...
    if both_have_general_sample(home, away):
        ...
        if all(value is not None for value in success_values):
            home_sr, away_def_sr, away_sr, home_def_sr = success_values
        ...
        explosive_values = [
            optional_number(home_off.get("explosive_rate")),
            optional_number(away_def.get("explosive_rate")),
            optional_number(away_off.get("explosive_rate")),
            optional_number(home_def.get("explosive_rate")),
        ]

        if all(value is not None for value in explosive_values):
            (
                home_explosive,
                away_def_explosive,
                away_explosive,
            ...
            components["explosiveness"] = bounded(
                (
                    (home_explosive - away_def_explosive)
                    - (away_explosive - home_def_explosive)
                )
                * 0.025,
                0.50,
            )
        ...
        if all(value is not None for value in havoc_values):
            (
                home_allowed,
                home_created,
                away_allowed,
    ...
    comparable = any(availability.values())
...
def calculate_win_probability(home_spread):
    ...
    return {
        "home": round(probability * 100, 1),
        "away": round((1.0 - probability) * 100, 1),
        "method": (
...
def fcs_fallback(
    ...
    return {
        ...
        "rating_only_margin": round(rating_only_margin, 1),
...
def fcs_win_probability(
    ...
    # Backward-compatible wrapper for the season projection code.
    return fcs_fallback(
        team_power_rating,
        rating_slope,
...
def generate_insights(
    ...
    if home_rank and away_rank and home_rank != away_rank:
        stronger = home_name if home_rank < away_rank else away_name
    ...
    if not matchup["comparable_live_sample"]:
        ...
        usable = [
            (key, value)
            for key, value in matchup["components"].items()
            if (
        ...
        for key, value in usable[:2]:
            favored = home_name if value > 0 else away_name
    ...
    if market_spread is not None:
        ...
        if abs(difference) >= 1.5:
            favored = home_name if difference < 0 else away_name
...
def generate_fcs_insights(
    ...
    insights = [
        {
            "title": "FCS FALLBACK MODEL",
            "text": (
                f"{fcs_name} does not have a full Hammer Index FBS rating. "
                f"The fair line uses the generic FCS baseline plus "
                f"{fbs_name}'s current FBS power rating."
            ),
            "source": "fcs_fallback",
        },
        {
            "title": "FCS TOTAL WITHHELD",
            "text": (
                "No opponent-specific model total is published for this game "
                "because the FCS opponent does not have the full efficiency "
                "profile required by the total model."
            ),
            "source": "fcs_total_guard",
        },
    ]
    ...
    if market_spread is not None:
        ...
        if abs(difference) >= 1.5:
            favored = (
                fbs_name
                if (
                    (fbs_is_home and difference < 0)
                    or (not fbs_is_home and difference > 0)
                )
                else fcs_name
            )

            insights.append({
                "title": "MARKET VS FCS FALLBACK",
                "text": (
                    f"The fallback line differs from the current market by "
                    f"{abs(difference):.1f} points toward {favored}."
                ),
                "source": "market",
            })
...
def build_odds_lookup(odds):
    ...
    for game in odds.get("games", []):
        ...
        if home and away:
            lookup[(home, away)] = game

    return lookup


def market_payload(market):
    if not market:
        return {
            "home_spread": None,
            "total": None,
            "bookmaker": None,
            "method": "cross_book_median_consensus",
            "spread_books": 0,
            "total_books": 0,
            "spread_outliers_removed": [],
            "total_outliers_removed": [],
    ...
    return {
        "home_spread": market.get("spread_home"),
        "total": market.get("total"),
        "bookmaker": market.get("bookmaker"),
        "method": "cross_book_median_consensus",
        "spread_books": market.get("spread_books", 0),
        "total_books": market.get("total_books", 0),
        "spread_outliers_removed": (
            market.get("spread_outliers_removed", [])
            or []
        ),
        "total_outliers_removed": (
...
def build_fbs_fbs_projection(
    ...
    rating_points = rating_difference * slope
    rating_home_spread = -rating_points
    ...
    spread_after_hfa = rating_home_spread - hfa

    matchup = calculate_matchup_adjustment(home, away)

    model_spread = round_half(
        spread_after_hfa - matchup["total"]
    )

    model_total = calculate_total(home, away)
    win_probability = calculate_win_probability(model_spread)

    market = odds_lookup.get((home_name, away_name))
    market_data = market_payload(market)

    comparison = compare_to_market(
        model_spread,
        market_data["home_spread"],
        home_name,
        away_name,
    )
    ...
    return {
        ...
        "projection": {
            ...
            "components": {
                "home_power_rating": round(home_rating, 3),
                "away_power_rating": round(away_rating, 3),
                "rating_difference": round(rating_difference, 3),
                "rating_points_home_edge": round(rating_points, 2),
                "rating_only_home_spread": round_half(
                    rating_home_spread
                ),
                "home_field_advantage": round(hfa, 1),
                "home_field_source": (
                    "neutral_site"
                    if neutral
                    else "team_specific_2026"
                ),
                "spread_after_home_field": round_half(
                    spread_after_hfa
                ),
                "matchup_adjustment": matchup,
                "final_home_spread": model_spread,
            },
            "base_home_spread": round_half(spread_after_hfa),
            "home_field_advantage": round(hfa, 1),
            "home_field_source": (
                "neutral_site"
                if neutral
...
def build_fbs_fcs_projection(
    ...
    home_is_fbs = home_name in teams
    away_is_fbs = away_name in teams

    # Exactly one side must be FBS.
    if home_is_fbs == away_is_fbs:
        return None

    fbs_is_home = home_is_fbs
    fbs_name = home_name if fbs_is_home else away_name
    fcs_name = away_name if fbs_is_home else home_name

    fbs_team = teams[fbs_name]
    fbs_rating = safe_number(fbs_team.get("power_rating"))

    neutral = bool(game.get("neutral_site", False))

    # Preserve the existing season-fallback rule:
    # FBS team-specific HFA is added only when the FBS team is the true home team.
    hfa = (
        get_team_hfa(
            fbs_name,
            hfa_data,
        ...
        if fbs_is_home and not neutral
        else 0.0
    )

    fallback = fcs_fallback(
        fbs_rating,
        slope,
        hfa_points=hfa,
    )
    ...
    rating_only_home_spread = (
        -fallback["rating_only_margin"]
        if fbs_is_home
        else fallback["rating_only_margin"]
    )

    model_spread = round_half(
        -fallback["expected_margin"]
        if fbs_is_home
        else fallback["expected_margin"]
    )

    fbs_probability = fallback["probability"]

    win_probability = {
        "home": (
            fbs_probability
            if fbs_is_home
            else round(100.0 - fbs_probability, 1)
        ),
        "away": (
            round(100.0 - fbs_probability, 1)
            if fbs_is_home
            else fbs_probability
        ),
        "method": "generic_fcs_fallback",
    }

    matchup = empty_matchup_adjustment(
        "FCS fallback game; opponent-specific live matchup adjustment withheld."
    )
    ...
    # The backend makes the fallback status explicit so downstream tracking
    # can exclude it even though the board can still display the market gap.
    comparison["model_type"] = "fcs_fallback"
    comparison["tracking_eligible"] = False
    comparison["bet_status"] = "UNTRACKED"

    home_data = (
        {
            "team": home_name,
            "conference": fbs_team.get("conference"),
            "power_rating": fbs_rating,
            "power_rating_rank": fbs_team.get("power_rating_rank"),
            "classification": "FBS",
        }
        if fbs_is_home
        else {
    ...
    return {
        ...
        "projection": {
            ...
            # Deliberately withheld. The full total model requires both teams'
            # efficiency profiles; the FCS opponent does not have one here.
            "total": None,
            ...
            "components": {
                ...
                "away_power_rating": (
                    None
                    if fbs_is_home
                    else round(fbs_rating, 3)
                ),
                "rating_difference": None,
                "rating_points_home_edge": None,
                "rating_only_home_spread": round_half(
                    rating_only_home_spread
                ),
                "home_field_advantage": round(hfa, 1),
                "home_field_source": (
                    ...
                    if fbs_is_home and not neutral
                        ...
                        "neutral_site"
                        if neutral
                        else "fcs_fallback_no_fbs_home_hfa"
                    )
                ),
                "spread_after_home_field": model_spread,
                "matchup_adjustment": matchup,
                "final_home_spread": model_spread,
                "fcs_fallback": {
                    "fbs_team": fbs_name,
                    "fcs_team": fcs_name,
                    "base_margin": FCS_BASE_MARGIN,
                    "power_multiplier": FCS_POWER_MULTIPLIER,
                    "rating_only_margin_for_fbs": fallback[
                        "rating_only_margin"
                    ],
                    "expected_margin_for_fbs": fallback[
                        "expected_margin"
                    ],
                    "method": fallback["method"],
                    "opponent_specific_fcs_rating": False,
                    "model_total_withheld": True,
                },
            },
        ...
        "insights": generate_fcs_insights(
            fbs_name,
            fcs_name,
            fbs_is_home,
            fallback,
...
def build_projections(
    ...
    for game in schedule.get("games", []):
        projection = build_fbs_fbs_projection(
            game,
            teams,
            odds_lookup,
            slope,
        ...
        if projection["market"]["home_spread"] is not None:
            market_matches += 1

        if projection["projection"]["matchup_adjustment"][
            "comparable_live_sample"
        ]:
            comparable_matchups += 1

    fcs_projection_count = 0
    fcs_market_matches = 0

    for game in collect_fcs_games(schedule):
        projection = build_fbs_fcs_projection(
            game,
            teams,
            odds_lookup,
            slope,
        ...
        projections.append(projection)
        fcs_projection_count += 1

        if projection["market"]["home_spread"] is not None:
            market_matches += 1
            fcs_market_matches += 1

    projections.sort(
        key=lambda game: (
            game.get("week") if game.get("week") is not None else 99,
            game.get("start_date") or "",
            game.get("away", {}).get("team") or "",
            game.get("home", {}).get("team") or "",
        )
    )

    print(f"✅ Total projections built: {len(projections)}")
    print(
        f"   FBS-vs-FBS projections: "
        f"{len(projections) - fcs_projection_count}"
    )
    print(
        f"   FBS-vs-FCS fallback projections: "
        f"{fcs_projection_count}"
    )
    print(
    ...
    print(
        f"   FBS-vs-FCS market matches: "
        f"{fcs_market_matches}"
    )
    print(
        "✅ Games with comparable live matchup samples: "
...
def completed_team_result(team, game):
    ...
    home_points = optional_number(game.get("home_points"))
    away_points = optional_number(game.get("away_points"))
    ...
    return 1.0 if winner == team else 0.0
...
def probability_at_least(distribution, wins):
    if wins <= 0:
        return 1.0
...
def alt_total_probabilities(distribution):
    ...
    for wins in range(4, max_wins + 1):
        line = wins + 0.5
        ...
        output[f"{line:.1f}"] = {
            "over": round(over_probability * 100, 1),
            "under": round(under_probability * 100, 1),
        }
...
def build_season_projections(
    ...
    print("")
    print("📈 Building season win distributions...")

    game_lookup = build_projection_lookup(projections)
    slope = calibration["slope"]
    season_output = {}
    ...
    for team_name, team in teams.items():
        ...
        for game in games:
            completed_result = completed_team_result(
                team_name,
                game,
            )
            ...
            if completed_result is not None:
            ...
            elif game.get("opponent_type") == "FBS":
                ...
                if projection["home"]["team"] == team_name:
                    probability = (
                ...
                hfa_used = projection[
                    "projection"
                ].get("home_field_advantage")

                source = "fbs_model"

            else:
                # Use the same FCS fallback assumptions as the public FBS-v-FCS row.
                projection = game_lookup.get(
                    str(game.get("id"))
                )
                ...
                if projection is not None:
                    ...
                    fallback = fcs_win_probability(
                        team.get("power_rating"),
                        slope,
                        hfa_points=hfa_used,
                    )
            ...
            game_probabilities.append(probability)
            ...
            schedule_projection.append({
                ...
                "team_line": (
                    round_half(model_line)
                    if model_line is not None
                    else None
                ),
                "home_field_advantage_used": hfa_used,
        ...
        exact_wins = {
            str(wins): round(probability * 100, 1)
            for wins, probability
            in enumerate(distribution)
        }

        expected_wins = sum(game_probabilities)

        most_likely_wins = max(
            range(len(distribution)),
            key=lambda wins: distribution[wins],
        )

        games_count = len(game_probabilities)
        most_likely_losses = (
            games_count
            - most_likely_wins
        )
        ...
        season_output[team_name] = {
            ...
            "most_likely_probability": round(
                distribution[most_likely_wins]
                * 100,
                1,
            ),
            "exact_win_distribution": exact_wins,
            "at_least": {
                "6_wins": at_least["6"],
                "8_wins": at_least["8"],
                "9_wins": at_least["9"],
            ...
            "bowl_eligible_probability": at_least["6"],
            "alt_win_totals": alt_total_probabilities(
                distribution
            ),
...
def validate_projection_output(
    ...
    if len(projections) < 400:
        print("❌ Too few game projections.")
        sys.exit(1)

    if len(season_projections) < 100:
        print("❌ Too few season projections.")
        sys.exit(1)

    bad_distributions = []

    for team, data in season_projections.items():
    ...
    if bad_distributions:
        ...
        print(bad_distributions[:10])
        sys.exit(1)

    bad_fcs_rows = []

    for game in projections:
        if game.get("model_type") != "fcs_fallback":
            continue

        levels = {
            game.get("home", {}).get("classification"),
            game.get("away", {}).get("classification"),
        }
    ...
    if bad_fcs_rows:
        print(
            "❌ FCS fallback classification "
            "sanity check failed."
        )
        print(bad_fcs_rows[:10])
        sys.exit(1)

    print("✅ Projection sanity checks passed")
...
def main():
    print("=" * 70)
    print("🏈 CFB ANALYTICS — PROJECTION ENGINE")
    print("=" * 70)
    print(f"Season: {YEAR}")

    metrics_data, teams = load_metrics()

    valid_teams = set(teams.keys())
    fbs_lookup = build_team_lookup(valid_teams)

    hfa_data = load_hfa(valid_teams)
    calibration = calculate_rating_scale(teams)

    # IMPORTANT:
    # Schedule must exist BEFORE odds matching so the resolver knows every
    # scheduled FCS opponent and can choose the longest true school match.
    use_cached_schedule = (
        str(
            os.environ.get(
                "USE_CACHED_SCHEDULE",
    ...
    verify_known_schedule_errors(schedule)

    if not use_cached_schedule:
        save_json(
            SCHEDULE_PATH,
            schedule,
        )

    fcs_lookup = build_fcs_lookup(schedule)

    print("")
    print(
        f"🏷️ Scheduled FCS opponent names loaded: "
        f"{len(fcs_lookup)}"
    )
    ...
    # Never replace a working public board with an empty provider response.
    # An empty pull can mean an exhausted quota, an invalid key, provider
    # downtime, or temporary coverage loss. Fail before writing odds or
    # projections so GitHub Actions leaves the last known-good pair intact.
    odds_games = odds.get("games", []) if isinstance(odds, dict) else []
    matched_games = (
        odds.get("meta", {}).get("matched_games", 0)
        if isinstance(odds, dict)
        else 0
    )

    if not odds_games or int(matched_games or 0) <= 0:
        print(
            "❌ Odds refresh returned no usable matched games. "
            "Preserving the existing odds.json and projections.json."
        )
        sys.exit(1)
    ...
    season_projections = build_season_projections(
        teams,
        schedule,
        projections,
        calibration,
    ...
    output = {
        "meta": {
            ...
            "fbs_fbs_games": (
                len(projections)
                - fcs_projection_count
            ),
            "fbs_fcs_fallback_games": fcs_projection_count,
            "teams_with_season_projection": len(
                season_projections
            ),
            "version": "3.4-fcs-fallback-board",
            "calibration": calibration,

            "market": {
                "method": "cross_book_median_consensus",
                "participant_matching": (
                    "longest_known_school_match_"
                    "fbs_plus_scheduled_fcs"
                ),
            ...
            "home_field_advantage": {
                "type": "team_specific",
                "source": HFA_PATH,
                "version": hfa_meta.get("version"),
                "default": safe_number(
                    hfa_meta.get("default_hfa"),
                    2.0,
                ),
                "minimum": safe_number(
                    hfa_meta.get("minimum_hfa"),
                    1.0,
                ),
                "maximum": safe_number(
                    hfa_meta.get("maximum_hfa"),
                    3.5,
                ),
                "neutral_site": safe_number(
                    hfa_meta.get("neutral_site_hfa"),
                    0.0,
                ),
                "situational_modifiers_included": False,
            },
            ...
            "fcs_model": {
                ...
                "opponent_specific_fcs_rating": False,
                "model_total_published": False,
                "tracking_eligible": False,
                "base_margin": FCS_BASE_MARGIN,
                "power_multiplier": FCS_POWER_MULTIPLIER,
                "minimum_win_probability":
                    FCS_MIN_WIN_PROB * 100,
                "maximum_win_probability":
                    FCS_MAX_WIN_PROB * 100,
                "home_hfa_rule": (
                    "FBS team-specific HFA is added only "
                    "when the FBS team is the true home team; "
                    "neutral/away receives 0.0."
                ),
                "note": (
                    "Generic fallback is used for FBS-v-FCS "
                    "fair spreads and win probability. "
                    "No opponent-specific FCS power rating or "
                    "model total is claimed."
                ),
            },

            "live_matchup_rules": {
                "minimum_plays": MIN_LIVE_PLAYS,
                "minimum_pass_plays":
                    MIN_LIVE_PASS_PLAYS,
                "minimum_rush_plays":
                    MIN_LIVE_RUSH_PLAYS,
