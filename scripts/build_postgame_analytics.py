...
def direction_value(value: Any) -> str | None:
...
def depth_value(value: Any) -> str | None:
    text = str(value or "").strip().lower()
    if text in {"short", "behind line", "behind_line", "screen"}:
        return "short"
    if text in {"deep", "long"}:
        return "deep"
    return None


def text_direction(play_text: str) -> str | None:
    match = re.search(r"\b(left|middle|right)\b", play_text.lower())
    return direction_value(match.group(1)) if match else None


def text_depth(play_text: str) -> str | None:
    match = re.search(r"\b(short|deep)\b", play_text.lower())
    return depth_value(match.group(1)) if match else None
...
def normalize_play(row: pd.Series, row_index: int) -> dict[str, Any] | None:
    ...
    pass_direction = direction_value(
        first_present(row, "pass_direction", "passDirection")
    )
    rush_direction = direction_value(
        first_present(row, "rush_direction", "rushDirection")
    )
    pass_depth = depth_value(
        first_present(row, "pass_depth", "passDepth")
    )
    if is_pass:
        pass_direction = pass_direction or text_direction(play_text)
        pass_depth = pass_depth or text_depth(play_text)
    if is_rush:
        rush_direction = rush_direction or text_direction(play_text)

    completion_raw = first_present(
        row,
        "complete_pass",
        "pass_complete",
        "completion",
    ...
    if completion_raw is not None:
        completion = as_bool(completion_raw)
    else:
        completion = bool(
            is_pass
            and "complete" in lower_blob
            and "incomplete" not in lower_blob
        )
    ...
    return {
        ...
        "air_yards": clean_number(first_present(row, "air_yards", "airYards")),
        "yards_after_catch": clean_number(first_present(row, "yards_after_catch", "yardsAfterCatch")),
...
def directional_summary(
    pass_plays: list[dict[str, Any]],
    rush_plays: list[dict[str, Any]],
) -> dict[str, Any]:
    ...
    for depth in ("short", "deep"):
        ...
        for direction in ("left", "middle", "right"):
            rows = [
                play for play in pass_plays
                if play.get("pass_depth") == depth
                and play.get("pass_direction") == direction
                and not play.get("sack")
            ]
            passing[depth][direction] = {
                "attempts": len(rows),
                "completions": sum(1 for play in rows if play.get("completion")),
                "yards": rnd(sum(float(play.get("yards_gained") or 0) for play in rows), 1),
                "epa_per_play": rnd(mean([float(play["epa"]) for play in rows]), 3),
                "success_rate": pct(mean([1.0 if play.get("success") else 0.0 for play in rows])),
            }

    rushing: dict[str, Any] = {}
    for direction in ("left", "middle", "right"):
        rows = [play for play in rush_plays if play.get("rush_direction") == direction]
        rushing[direction] = {
            "attempts": len(rows),
            "yards": rnd(sum(float(play.get("yards_gained") or 0) for play in rows), 1),
            "epa_per_play": rnd(mean([float(play["epa"]) for play in rows]), 3),
    ...
    charted_passes = sum(
        ...
        if play.get("pass_depth") in {"short", "deep"}
        and play.get("pass_direction") in {"left", "middle", "right"}
        and not play.get("sack")
    )
    charted_rushes = sum(
        1 for play in rush_plays
        if play.get("rush_direction") in {"left", "middle", "right"}
    )
    ...
    return {
        ...
        "rushing": rushing,
        "coverage": {
            "passing_charted": charted_passes,
            "passing_total": sum(1 for play in pass_plays if not play.get("sack")),
            "rushing_charted": charted_rushes,
            "rushing_total": len(rush_plays),
        },
    }
...
def build_full_record(
    ...
    def pack_team(stats: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "directional": stats.get("directional") or {
                "passing": {},
                "rushing": {},
                "coverage": {},
            },
    ...
    return {
        ...
        "definitions": {
            ...
            "directional_chart": (
                "Pass depth/direction and rush direction are parsed from provider fields or play text. "
                "Unclassified plays remain outside the heat map and are disclosed in chart coverage."
            ),
            "expected_margin": (
...
def main(force_refresh: bool = False) -> None:
    ...
    if not force_refresh and not should_download(
        finals,
        existing,
    ):
        print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--refresh",
        action="store_true",
        help="Rebuild all existing final-game records from the current PBP source.",
    )
    arguments = parser.parse_args()
    main(force_refresh=arguments.refresh)
